...
3. Згенеруй урок за `../practicum/lesson-format.md` (прочитай його):
   пояснення, покрокові нотатки, lesson-JSON, перевірка офсетів,
   збирання HTML.
4. Ім'я файла: `course-NN-<slug>.html` (NN — номер уроку, slug —
   латиницею з дефісами).
5. Якщо у «Знахідках» вирішено «вчимо як є» — нотатки уроку чесно
   згадують кращу практику там, де код її порушує.
6. Відміть урок `[x]` у course.md.
